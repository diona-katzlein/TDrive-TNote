 ...
 */
async function createNote(accountId, { title, body, category, peer, note_type }, encryptionKey) {
  const ts = now();
  ...
  const [result] = await db.query(
    'INSERT INTO notes (account_id, title, body, category, peer, uuid, note_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [accountId, encTitle, encBody, encCategory, peer || 'me', u, note_type || 'markdown', ts, ts]
  );
 ...
 */
async function updateNote(id, { title, body, category, peer, note_type }, encryptionKey) {
  const encTitle = noteCrypto.encrypt(title || 'Tanpa Judul', encryptionKey);
  const encBody = noteCrypto.encrypt(body || '', encryptionKey);
  const encCategory = noteCrypto.encrypt(category || 'General', encryptionKey);

  return db.query(
    'UPDATE notes SET title = ?, body = ?, category = ?, peer = ?, note_type = ?, updated_at = ? WHERE id = ?',
    [encTitle, encBody, encCategory, peer || 'me', note_type || 'markdown', now(), id]
  );
}
